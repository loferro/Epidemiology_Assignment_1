...
#### Gaussian
\
Let's first try using the Gaussian model to do our prediction step:
...
image(kc.gaussian,xlim=range(poly84$x), ylim=range(poly84$y), borders = poly84, coords.data = as.matrix(dataset.new[,c(1,2)]), main = "Predicted elevation of the islet")
```

```{r  out.width = '60%'}
contour(kc.gaussian,filled= TRUE,val=sqrt(kc.gaussian$krige.var), coords.data=geodataset$coords,xlim=range(poly84$x),ylim=range(poly84$y), col = gray(seq(1, 0.1, l = 30)), main="Standard Deviation of the prediction",borders = poly84)
```
...
#### Matern 
\
Let's now use the Matern model
...
image(kc.matern, xlim=range(poly84$x),ylim=range(poly84$y), borders = poly84, coords.data = as.matrix(dataset.new[,c(1,2)]), main = "Predicted elevation of the islet")
```
```{r  }
#contour(kc.matern,filled= TRUE, coords.data=geodataset$coords,xlim=range(poly84$x),ylim=range(poly84$y), col=terrain.colors(30), borders = poly84)
```

```{r  out.width = '60%'}
contour(kc.matern,filled= TRUE,val=sqrt(kc.matern$krige.var),
coords.data=geodataset$coords,xlim=range(poly84$x),ylim=range(poly84$y), col = gray(seq(1, 0.1, l = 30)), borders = poly84[,1:3], main  = "Standard Deviation of the prediction")
```
...
Look at the performance of the two model through 3 goodness indexes:
